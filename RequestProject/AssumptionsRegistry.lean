...
def assumptionsRegistry : List RegistryEntry := [
  ...
  -- ParityBudget.Setup
  { name := "parityWitness"
    module := "Core.ParityBudget.Setup"
    status := .ProvedInProject
    description := "Finite parity witness χ(x) = μ(gcd(x, P))" },

  -- ParityBudget.AbstractBudget
  { name := "target_correlation_le_majorant_correlation_plus_surplus"
    module := "Core.ParityBudget.AbstractBudget"
    status := .ProvedInProject
    description := "Target correlation ≤ majorant correlation + surplus" },
  { name := "parity_separation_implies_surplus"
    module := "Core.ParityBudget.AbstractBudget"
    status := .ProvedInProject
    description := "Parity separation implies mass surplus" },
  { name := "budget_from_parity_separation"
    module := "Core.ParityBudget.AbstractBudget"
    status := .ProvedInProject
    description := "Mass lower bound from parity separation" },
  { name := "energy_budget_from_parity_separation"
    module := "Core.ParityBudget.AbstractBudget"
    status := .ProvedInProject
    description := "Energy budget from parity separation" },

  -- ParityBudget.FiniteParityProxy
  { name := "parityWitness_bounded"
    module := "Core.ParityBudget.FiniteParityProxy"
    status := .ProvedInProject
    description := "|χ(x)| ≤ 1 for the parity witness" },
  { name := "parityWitness_coprime_eq_one"
    module := "Core.ParityBudget.FiniteParityProxy"
    status := .ProvedInProject
    description := "χ(x) = 1 when gcd(x, P) = 1" },
  { name := "parityWitness_target_correlation"
    module := "Core.ParityBudget.FiniteParityProxy"
    status := .ProvedInProject
    description := "Σ_{coprime x} χ(x) = φ(P) * m" },

  -- ParityBudget.MoebiusCase (Result A — sorry-free)
  { name := "parity_budget_moebius"
    module := "Core.ParityBudget.MoebiusCase"
    status := .ProvedInProject
    description := "Parity budget theorem at Möbius weights (Result A, sorry-free)" },
  { name := "parity_separating_weights_cost_more_than_moebius"
    module := "Core.ParityBudget.MoebiusCase"
    status := .ProvedInProject
    description := "Parity-separating weights have Q ≥ Q(μ) (sorry-free)" },

  -- ParityBudget.Examples
  { name := "parityWitness_sum_six"
    module := "Core.ParityBudget.Examples"
    status := .ProvedInProject
    description := "Σ χ(x) = 0 for P=6, N=6 (sanity check)" },
  { name := "parityWitness_target_six"
    module := "Core.ParityBudget.Examples"
    status := .ProvedInProject
    description := "Coprime correlation = 2 for P=6 (sanity check)" },

  -- ParityBudget.GeneralCase (Result B — conditional on optimalWeight_quadForm_eq)
  { name := "optimalWeights_minimize_quadForm"
    module := "Core.ParityBudget.GeneralCase"
    status := .ScaffoldedNotProved
    description := "Optimal weights minimize Q(λ) — conditional on IK eq. 6.70" },
  { name := "parity_budget_general"
    module := "Core.ParityBudget.GeneralCase"
    status := .ScaffoldedNotProved
    description := "Parity budget for general g (Result B, conditional)" },
  { name := "parity_separating_weights_exceed_optimal"
    module := "Core.ParityBudget.GeneralCase"
    status := .ScaffoldedNotProved
    description := "Parity-separating weights exceed optimal (conditional)" }
]
